...
# ---------- UPDATE FUNCTION ----------
def update():
    global current_time_s, y_theory, ser, K, TAU, VSTART, OFFSET
    ...
    while ser.in_waiting >= PACK_SIZE:
        ...
        y_theory += dt_s * dy
        y_theory_offset = y_theory + OFFSET

        # ---- Window data ----
        t_data.append(current_time_s)
        vel_data.append(vel)
        ref_data.append(ref)
        theory_data.append(y_theory_offset)

        # ---- Full data ----
        full_time.append(current_time_s)
        full_vel.append(vel)
        full_ref.append(ref)
        full_theory.append(y_theory_offset)
...
def update_params():
    global K, TAU, VSTART, OFFSET, y_theory
    try:
        K = float(edit_K.text())
...
main.setWindowTitle("RP2040 Velocity Logger")
...
ctrl.addWidget(QtWidgets.QLabel("VSTART:"), 1, 4)
ctrl.addWidget(edit_VSTART, 1, 5)
ctrl.addWidget(btn_apply, 1, 6)

ctrl.addWidget(btn_csv_win, 2, 0)
ctrl.addWidget(btn_mat_win, 2, 1)
ctrl.addWidget(btn_csv_all, 3, 0)
ctrl.addWidget(btn_mat_all, 3, 1)

layout.addLayout(ctrl)

# ---------- PLOT ----------
plot = pg.PlotWidget(title="Velocity (yellow) | Reference (cyan) | Theory+Offset (magenta)")
